	...
	if (go.isa("figure")) {
		...
		for (auto ax : children(go))
			if (ax.isa("axes")) {
				...
				text::properties& ylabelProperties = dynamic_cast<text::properties&>(ylabel.get_properties());
				text::properties& zlabelProperties = dynamic_cast<text::properties&>(zlabel.get_properties());
				...
				if (!ax.get("tag").isempty() && ax.get("tag").string_value() == "polaraxes") {
					...
					polarAxis(plot["layout"][p]["radialaxis"],
							  axisProperties.get("rtick").matrix_value(),
							  axisProperties.get_fontsize());

					polarAxis(plot["layout"][p]["angularaxis"],
					...
					if (xlabel && xlabel.isa("text")) text(plot["layout"][x]["title"],
					...
					if (ylabel && ylabel.isa("text")) text(plot["layout"][y]["title"],
					...
					// Adding labels
					if (xlabel && xlabel.isa("text")) text(plot["layout"][s]["xaxis"]["title"],
														   xlabelProperties.get_string().string_value(),
														   xlabelProperties.get_interpreter(),
														   xlabelProperties.get_color_rgb(),
														   xlabelProperties.get_fontsize());

					if (ylabel && ylabel.isa("text")) text(plot["layout"][s]["yaxis"]["title"],
														   ylabelProperties.get_string().string_value(),
														   ylabelProperties.get_interpreter(),
														   ylabelProperties.get_color_rgb(),
														   ylabelProperties.get_fontsize());

					if (zlabel && zlabel.isa("text")) text(plot["layout"][s]["zaxis"]["title"],
				...
				for (auto d : children(ax)) {
					...
					if (d.isa("line")) {
						line::properties& lineProperties = dynamic_cast<line::properties&>(d.get_properties());
						std::string type;
						...
						setLegendVisibility(plot["data"][dNumber],
											lineProperties.get_displayname());
					} else if (d.isa("surface")) {
						surface::properties& surfaceProperties = dynamic_cast<surface::properties&>(d.get_properties());
						...
						if (axisProperties.get_is2D()) {
							...
							setLegendVisibility(plot["data"][dNumber],
												surfaceProperties.get_displayname());
						}
					} else if (d.isa("text")) {
						text::properties& textProperties = dynamic_cast<text::properties&>(d.get_properties());
						...
						text(plot["layout"]["annotations"][aNumber],
						...
						hggroup::properties& hggroupProperties = dynamic_cast<hggroup::properties&>(d.get_properties());

						switch (components.size()) {
							case 2:
								// We suppose that a line+line hggroup is a stem
								if (components[0].isa("line") && components[1].isa("line")) {
									line::properties& lineProperties = dynamic_cast<line::properties&>(components[0].get_properties());
									std::string type;
						...
						setLegendVisibility(plot["data"][dNumber],
											hggroupProperties.get_displayname());
					}
				}
			}
		...
		dynamic_cast<xoctave_interpreter&>(xeus::get_interpreter()).do_update_display_data(data, json::object(), tran);
	}
}
...
void plotly_graphics_toolkit::show_figure(const graphics_object& go) const {
	...
	json tran;
	tran["display_id"] = id;
	dynamic_cast<xoctave_interpreter&>(xeus::get_interpreter()).do_display_data(json::object(), json::object(), tran);
}
...
std::string plotly_graphics_toolkit::convertText(std::string text, std::string format) const {
	if (format == "latex") {
		return "$ " + text + " $";
	} else if (format == "tex") {
...
void plotly_graphics_toolkit::text(json& obj,
								   std::string text,
...
void plotly_graphics_toolkit::axis(json& axis,
								   bool visible,
								   std::string scale,
								   std::string location,
								   Matrix range,
	...
	if (showMinorTicks) {
		...
		for (size_t i = 0; i < allTicks.size(); i++) {
			if (std::count(ticks.begin(), ticks.end(), allTicks[i])) {
				int index = std::distance(ticks.begin(), std::find(ticks.begin(), ticks.end(), allTicks[i]));
...
void plotly_graphics_toolkit::polarAxis(json& axis,
										Matrix _ticks,
										double fontSize) const {
...
void plotly_graphics_toolkit::legend(json& legend,
									 Matrix position,
									 bool box,
									 double lineWidth,
...
void plotly_graphics_toolkit::line(json& line,
								   bool visible,
								   std::string type,
								   Matrix xdata,
								   Matrix ydata,
	...
	if (type == "scatterpolar") {
		...
		for (int i = 0; i < xdata.cols(); i++) {
			std::complex<double> vector(xdata(i), ydata(i));

			line["r"][i] = std::abs(vector);
			line["theta"][i] = std::arg(vector);
		}
		...
		for (int i = 0; i < ydata.cols(); i++)
			line["y"][i] = ydata(i);

		for (int i = 0; i < zdata.cols(); i++)
			line["z"][i] = zdata(i);
	}
}

void plotly_graphics_toolkit::surface(json& surf,
									  bool visible,
									  Matrix xdata,
									  Matrix ydata,
									  Matrix zdata,
	...
	for (int i = 0; i < ydata.rows(); i++)
		surf["y"][i] = ydata(i, 0);

	for (int i = 0; i < zdata.rows(); i++)
		for (int j = 0; j < zdata.cols(); j++)
			surf["z"][i][j] = zdata(i, j);

	for (int i = 0; i < cdata.rows(); i++)
		for (int j = 0; j < cdata.cols(); j++)
			surf["surfacecolor"][i][j] = cdata(i, j);

	for (int i = 0; i < colorMap.rows(); i++) {
		surf["colorscale"][i][0] = ((double) i) / (colorMap.rows() - 1);
		surf["colorscale"][i][1] = matrix2rgb(colorMap.row(i));
	}
...
void plotly_graphics_toolkit::setLegendVisibility(json& data,
